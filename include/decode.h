...
uint8_t cpu_read_byte(cpu_state_t *cpu, uint8_t *memory) {
    return memory[cpu->registers.pc++];
}

uint16_t cpu_read_word(cpu_state_t *cpu, uint8_t *memory) {
    uint16_t address = cpu->registers.pc;
    uint16_t result = memory[address] | (memory[address + 1] << 8);
    //SDL_Log("Read word: %04X from address: %04X", result, address-1);
    cpu->registers.pc += 2;
    return result;
}

instruction_t cpu_decode_instruction(cpu_state_t *cpu, uint8_t *memory, uint8_t opcode) {
    ...
    switch (opcode) {
        ...
        case 0x06:
            ...
            instruction.byte_operand = cpu_read_byte(cpu, memory);
            break;
        case 0x0E:
            instruction.type = LD_nn_n;
        ...
        case 0x21:
            ...
            instruction.word_operand = cpu_read_word(cpu, memory);
            break;
